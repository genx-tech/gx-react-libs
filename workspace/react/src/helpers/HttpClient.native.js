...
class HttpClient {
    ...
    async _send(method, path, query, body, options, setRequestBody) {
        ...
        const url =
            path.startsWith('http:') || path.startsWith('https:') || !this.endpoint
                ? path // absolute url
                : urlUtil.join(options && options.endpoint ? options.endpoint : this.endpoint, path);
    ...
    _createRequest(httpMethod, url) {
        Runtime.log('verbose', `Http request sent: ${httpMethod} ${url}`);
