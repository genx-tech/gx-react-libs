...
class StyleXSheet {
    ...
    injectTheme() {
        ...
        this.customTheme != null &&
            this.insert(
                buildThemeVariables(
                    '.' + _CUSTOM_THEME_CLASS_NAME + ':root, .' + _CUSTOM_THEME_CLASS_NAME,
                    this.customTheme
                ),
                0
            );
    ...
    delete(rule) {
        ...
        if (this.isSlow) {
            tag.removeChild(tag.childNodes[ruleIndex + 1]);
        } else {
    ...
    normalizeRule(rule) {
        ...
        return this.supportsVariables || theme == null
            ? rule
            : rule.replace(VARIABLE_REGEX, function (a, c) {
                  return theme[c];
              });
    }
    ...
    getInsertPositionForPriority(priority) {
        ...
        let b = Array.from(this.ruleForPriority.keys())
            .sort(function (a, a2) {
                return a2 - a;
            })
            .filter(function (a) {
                return a > priority ? 1 : 0;
            });
        if (b.length === 0) {
    ...
    insert(themeVariables, priority) {
        // ensure stylesheet was injected
        this.injected === false && this.inject();

        if (this.externalRules.has(themeVariables.slice(0, themeVariables.indexOf('{')).trim())) {
            return;
        }
        if (this.rules.includes(themeVariables)) {
            return;
        }
        const a = this.normalizeRule(themeVariables);
        if (this.externalRules.has(a.slice(0, a.indexOf('{')).trim())) {
            return;
        }

        const insertIndex = this.getInsertPositionForPriority(priority);
        this.rules.splice(insertIndex, 0, a);
        ...
        if (this.isSlow) {
            ...
            tag.insertBefore(d, tag.childNodes[insertIndex]);
        } else {
            d = tag.sheet;
            if (d != null) {
                try {
                    d.insertRule(a, insertIndex);
                } finally {
                }
            }
        }
